...
PROGRAM QG_FFT
	...
	CHARACTER(LEN=200)					:: InputFile			! Variable that will contain the name of the file with the EOFs eigenvalues (and hence cumulated variance)	
	! 
	...
	Method = 'DMD'
	! Read the corresponding EOF amplitude
	...
	DO nit=niter+1, NMAX
				...
				SELECT CASE(dd)
					CASE(1)
						IF(mod(nit,nDMD)==0) THEN
							...
							IF(nit>(m+1)*nDMD) THEN
								! Propagate the DMD modes
								! Note that if ll==0, the DMD modes will be recomputed in the next IF-cycle; hence the next two lines will be overwritten before actually doing something
								CALL DMDCovariance(Csigma,RDMDs,IDMDs,omega,lambda_order,nstart,neigen,dt,ll,cum_var,NX,NY,r)
								CsCs = matmul(CSigma,transpose(Csigma))
							END IF
						END IF
						IF(mod(nit,mcycle*(m+1)*nDMD)==0) THEN 
							...
							!CALL DMDStructure(Csigma,NX,NY,nstart,neigen,m,r,dt*nTec,X1,X2)
							CALL DMDModes(RDMDs,IDMDs,omega,lambda,lambda_order,NX,NY,m,r,dt,X1,X2)
							CALL DMDCovariance(Csigma,RDMDs,IDMDs,omega,lambda_order,nstart,neigen,dt,1,cum_var,NX,NY,r)
							CsCs = matmul(CSigma,transpose(Csigma))
					...
					CASE(2)
						...
						IF(mod(nit,nDMD)==0) THEN
							...
							IF(nit>(dd*m)*nDMD) THEN
								! Propagate the DMD modes
								! Note that if ii==0, the DMD modes will be recomputed in the next IF-cycle; hence the next two lines will be overwritten before actually doing something
								CALL DMDCovariance(Csigma,RDMDs,IDMDs,omega,lambda_order,nstart,neigen,dt,ii,cum_var,NX,NY,r)
								CsCs = matmul(CSigma,transpose(Csigma))
							END IF
						END IF
		...
		ELSE
			LB1 = rhsB
		END IF
		! Compute friction and hyperviscosity 
		LqB1 = 0.0d0
		LqT1 = 0.0d0
		DO j=1, NY
			DO i=1, NX
