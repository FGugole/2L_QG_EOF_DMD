...
! Period parameters
    integer, parameter :: NN = 624, NN1 = NN + 1

! the array for the state vector
    integer, save, dimension(0:NN-1) :: mt
    integer, save                   :: mti = NN1
...
!                              a default initial seed is used
      ...
      do kk=NN-M,NN-2
          y=ior(iand(mt(kk),UMASK),iand(mt(kk+1),LMASK))
          mt(kk)=ieor(ieor(mt(kk+(M-NN)),ishft(y,-1)),mag01(iand(y,1)))
      enddo
      y=ior(iand(mt(NN-1),UMASK),iand(mt(0),LMASK))
      mt(NN-1)=ieor(ieor(mt(M-1),ishft(y,-1)),mag01(iand(y,1)))
      mti = 0
    endif
